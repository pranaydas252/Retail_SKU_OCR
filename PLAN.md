...
| ~~F3~~ | ~~The readiness gate passed a motion-blurred capture.~~ **Addressed 2026-08-17 by removing auto-capture.** Sample index 18 was unreadable and the shutter fired on it by itself. The gate is now advisory only — it colours the brackets and the hint, and the operator presses the button. A blurred frame can still be captured, but only because someone chose to, which is a different and much less serious failure than the app choosing for them. | `sample_data/roi/SAMPLE_20260816_200322_734.jpg` | Closed |
