...
| F7 | **Two dangling references were left by the corpus reset.** `images/` was cleared while `SkuScan` still held rows pointing at those files, breaking the section 16 audit trail for old scans. Truncation is queued as a manual step. | — | Low |
